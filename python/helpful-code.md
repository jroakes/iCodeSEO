...
### Text Mining SEO Data - Word Counting

Many SEO reports typically come as a text list \(page titles, URLs, keywords, snippets, etc.\) together with one or more number lists \(pageviews, bounces, conversions, time on page, etc.\). 

Looking at the top pages with disproportionately higher numbers might be misleading if you have a significant long tail of topics in the text list. How do you uncover hidden insights?   
Let's try to do that with the following report:
...
page_titles = [
    ...
    'Data Science for Marketing People',
    'Data Science for Business People',
    'Python for SEO',
    'SEO Text Analysis'
]

pageviews = [200, 150, 400, 300, 670, 120, 340]


pd.DataFrame(zip(page_titles, pageviews),
             columns=['page_titles', 'pageviews'])
```

![](../.gitbook/assets/screen-shot-2020-03-17-at-4.50.33-am.png)

It's clear which is the most viewed article. But are there any hidden insights about the words, and topics occurring in this report that we can't immediately see? 

```python
import advertools as adv
adv.word_frequency(page_titles, pageviews)
```

![](../.gitbook/assets/screen-shot-2020-03-17-at-4.53.01-am.png)

By counting the words in the titles we can get a better view. Counting word occurrences on an absolute basis \(simple counting\), we can see that "python" is the top topic, occurring four times.  
However, on a weighted count basis \(by taking into consideration the total pageviews generated by titles where the word was included\), "data" is the winner word. Pages containing "data" generated a total of 1,120 pageviews.   
And on a per-occurrence basis, the word "business" is the winner because it generated the most pageviews by appearing in only one page title. 

To summarize:   
- The topic that we wrote the most about was "python"  
- The topic that generated the most pageviews was "data"  
- The relatively most valuable topic was "business"  
